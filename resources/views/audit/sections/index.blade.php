        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalLongTitle">Раздел</h5>
                            ...
                            <div class="kt-section kt-section--first">
                                <div class="kt-section__body">
                                    <div class="form-group row">
                                        <label class="col-lg-3 col-form-label">Название раздела:</label>
                                        <div class="col-lg-9">
        ...
        let KTDatatablesDataSourceAjaxServer = function() {
            ...
            let initTable = function() {
                ...
                let DTtable = table.DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '{{ route('sections.index.datatable') }}',
                        headers: {
            ...
            let newPosition = function() {
                newButton.on('click', function (el) {
                    form.attr('action', '{{ route('sections.store') }}');
            ...
            let editPosition = function(DT_table) {
                table.on('click', '.positionsEdit', function(e){
                    let record_id = this.closest('tr').getAttribute('id').slice(4);
                    $.get("{{ route('sections.index') }}" +'/' + record_id, function (response) {
                        form.attr('action', '{{ route('sections.index') }}' + '/' + response.data.id);
                        let el = '<input type="hidden" name="section_id" value="'+response.data.id+'" />';
                        form.append(el);
