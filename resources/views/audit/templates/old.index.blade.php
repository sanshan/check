...
@section('content')
    ...
    @include('audit.templates._section-weight-edit')
...
@push('scripts')
    <script>
        ...
        let KTDatatablesDataSourceAjaxServer = function () {
            ...
            let templateSectionsList = $('#templateSectionsList');
            ...
            let weightEditModal = $('#templateSectionWeightEdit');
            let weightEditForm = $('form', weightEditModal);
            let weightEditSubmitButton = $('#weightEditFormSubmitButton', weightEditModal);
            ...
            let editPosition = function (typeOfGasStation, typeOfChecklist, regions) {
                table.on('click', '.positionsEdit', function (e) {
                    ...
                    $.get("{{ route('templates.index') }}" + '/' + record_id, function (response) {
                        ...
                        typeOfGasStation.val(null).trigger('change');
                        if (response.data.type_of_gas_station) {
                        ...
                        typeOfChecklist.val(null).trigger('change');
                        if (response.data.type_of_checklist) {
            ...
            let showPosition = function (DTtable) {
                table.on('click', '.model-show', function (e) {
                    ...
                    let templateSectionsDT = initTemplateSections(modelId);
                        ...
                        onBlock: function () {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    modalShow.find('#author').text(response.data.user.profile.full_name);
                                    ...
                                    modalShow.find('#showSectionsManagementModalButton').attr('data-id', response.data.id);
                                },
                                error: function (xhr, status, errorThrown) {
            ...
            let templateSectionModalClose = function (DTtable) {
                modalShow.on('hide.bs.modal', function (event) {
                    console.log('закрыл окно с разделами');
                    let api = $('#templateSectionsList').dataTable().api();
                    $(api.table().body()).off('click', '.edit-positions');
                    $(api.table().body()).off('click', '.edit-weight');
                    $('#templateSectionsList').DataTable().clear();
                    $('#templateSectionsList').DataTable().destroy();
                    DTtable.ajax.reload(null, false);
                });
            };

            let showTemplateQuestions = function (api, modelId) {
                $(api.table().body()).on('click', '.show-questions', function (event) {
                    event.preventDefault();
                    let section_id = event.target.dataset.sectionId;
                    let template_id = event.target.dataset.templateId;
                    console.log('Открыл окно с вопросами');
                    let templateQuestionsDT = initTemplateQuestions(template_id, section_id);
                    $('#templateShowQuestions').modal('show');
            ...
            let templateQuestionsModalClose = function () {
                $('#templateShowQuestions').on('hide.bs.modal', function (event) {
                    console.log('Закрыли окно  вопросами');
                    //templateSectionsList.DataTable().clear();
                    //templateSectionsList.DataTable().destroy();
                });
            };
            ...
            let initTemplateSections = function (modelId) {
                templateSectionsList.DataTable({
                    async: false,
                    responsive: true,
                    processing: true,
                    serverSide: true,
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            targets: 1,
                            render: function (data, type, full, meta) {
                                return '<a class="d-block show-questions" title="' + data + '" data-section-id="' + full.id + '" data-template-id="' +modelId+ '" href="">' + data + '</a>';
                            }
                        },
                        {
                            targets: -1,
                    ...
                    drawCallback: function (settings) {
                        let api = this.api();
                        editSectionWeight(api, modelId);
                        showTemplateQuestions(api, modelId)
                    },
                });
            };

            let initTemplateQuestions = function (template_id, section_id) {
                $('#templateQuestionsList').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '/api/templates/' + template_id + '/sections/' + section_id + '/questions',
                    },
                    order: [[0, 'asc']],
                    language: {
                        url: "//cdn.datatables.net/plug-ins/1.10.19/i18n/Russian.json",
            ...
            /*let showQuestions = function (api, modelId) {
                $(api.table().body()).on('click', '.show-questions', function (event) {
                    event.preventDefault();
                    let section_id = event.target.dataset.id;
                    $('#templateShowQuestions').modal('show', this);
                });
            };*/

            let editSectionWeight = function (api, modelId) {
                $(api.table().body()).on('click', '.edit-weight', function (event) {
                    let section_id = event.target.dataset.id;
                    weightEditForm.attr('action', '{{ route('templates.index') }}' + '/' + modelId + '/sections/' + section_id);
                    let currentWeight = event.target.text;
                    let positions = $('#sectionWeight', weightEditForm);
                    positions.val(currentWeight);

                    weightEditModal.modal('show');
                });
            };

            let weightEditFormSubmit = function () {
                weightEditSubmitButton.on('click', function (e) {
                    weightEditForm.submit();
                });
            };

            let weightEditFormValidate = function () {
                weightEditForm.validate({
                    rules: {
                        'weight': {
                            required: true,
                        },
                    ...
                    submitHandler: function () {
                        $(weightEditForm).ajaxSubmit({
                            url: weightEditForm.action,
                            type: 'POST',
                            success: function (response) {
                                let values = response.message;
                                templateSectionsList.DataTable().ajax.reload(null, false);
            ...
            return {
                init: function () {
                    ...
                    weightEditFormSubmit();
                    weightEditFormValidate();
                    ...
                    templateSectionModalClose(DTtable);
                    templateQuestionsModalClose();
                },
