    ...
    <div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
        <div class="alert alert-light alert-elevate" role="alert">
            ...
            <div class="alert-text">
                Управление справочником "Типы АЗС". <br>
                <u>Функционал:</u>
                <ul>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalLongTitle">Тип АЗС</h5>
                            ...
                            <div class="kt-section kt-section--first">
                                <div class="kt-section__body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-9">
                                            <input type="text" class="form-control" name ="abbreviation" placeholder="Введите тип">
        ...
        let KTDatatablesDataSourceAjaxServer = function() {
            ...
            let initTable = function() {
                ...
                let DTtable = table.DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '{{ route('typeofgasstations.index.datatable') }}',
                        headers: {
            ...
            let newPosition = function() {
                newButton.on('click', function (el) {
                    form.attr('action', '{{ route('typeofgasstations.store') }}');
                    modal.find('input[name=_method]').val('POST');
                    modal.find('input[name=title]').val('');
                    modal.find('input[name=abbreviation]').val('');
                    modal.modal('show');
                });
            };
            ...
            let deletePosition = function(DTtable) {
                ...
                table.on('click', '.positionsDelete', function(e){
                    ...
                    if (confirm("Удалить элемент")) {
                        $.ajax({
                            url: '/api/typeofgasstations/'+record_id,
                            method: 'POST',
                            data: {
                                _method: 'DELETE',
                            },
            ...
            let editPosition = function(DT_table) {
                table.on('click', '.positionsEdit', function(e){
                    let record_id = this.closest('tr').getAttribute('id').slice(4);
                    $.get("{{ route('typeofgasstations.index') }}" +'/' + record_id, function (response) {
                        form.attr('action', '{{ route('typeofgasstations.index') }}' + '/' + response.data.id);
                        let el = '<input type="hidden" name="type_of_gas_station_id" value="'+response.data.id+'" />';
                        form.append(el);
                        modal.find('input[name=_method]').val('PUT');
                        modal.find('input[name=title]').val(response.data.title);
                        modal.find('input[name=abbreviation]').val(response.data.abbreviation);
                        modal.modal('show');
                    })
                });
            };
